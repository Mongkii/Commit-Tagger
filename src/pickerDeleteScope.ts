...
export function openDeletePicker(showPrevPicker: () => void, context: ExtensionContext): void {
  ...
  picker.onDidAccept(() => {
    ...
    const newScopeNames = scopeNames.filter((item) => !toDeleteScopeSet.has(item.label));
